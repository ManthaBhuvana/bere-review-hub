...
//
// These four entries are the real, published pieces of Volume 1, Issue 1 — the
// full review issue, the editorial, the foreword, and the leader's
// toolkit. There are no other essays yet; add them here as they're written.
//
// Display dates come from ISSUE_DATE in src/lib/site.ts (one place to update).
// Founder & Editor name/title come from FOUNDER_NAME / FOUNDER_TITLE.

import { FOUNDER_NAME, FOUNDER_TITLE, ISSUE_DATE, SITE_NAME } from "../lib/site";

// PDFs live in /public/images/pdfs and are served from your own domain.
const STORAGE_BASE = "/images/pdfs";
...
export interface Article {
  ...
  author: string;
  authorRole?: string;
  date: string; // Display date, e.g. "October 2026" (see ISSUE_DATE in src/lib/site.ts)
...
// "Venugopal Bere" -> "Bere, V." (APA author format, matching the Publications page).
...
export function formatCitation(article: Article): string {
  ...
  const volume = article.volume.replace(/\s*\(\d{4}\)/, "");
  return `${toApaName(article.author)} (${year}). ${article.title}. ${SITE_NAME}, ${volume}.`;
}
